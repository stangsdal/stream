...
// function code 0x43
Stream.prototype.jablotronReadRegisterFromIndex = function (options, next) {
	options = options || {};

	var read_category  = (typeof options.read_category != "undefined" ? options.read_category : 0);
	var read_index = (typeof options.read_index != "undefined" ? options.read_index : 1);
	var read_page = (typeof options.read_page != "undefined" ? options.read_page : 0);
	var read_quantity = (typeof options.read_quantity != "undefined" ? options.read_quantity : 0);
	var values        = (typeof options.values != "undefined" ? options.values : buffer.from([ 0, 0 ]));
	var extra         = options.extra || {};
	var next          = next || null;

	this.transport.send("JablotronReadRegisterFromIndex", extra, next, read_category, read_index, read_page, read_quantity, values);
};
